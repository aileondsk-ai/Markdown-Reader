...
export default function Test() {
  ...
  // 결과 화면으로 전환 시 포커스를 결과 영역으로 이동 (접근성)
  useEffect(() => {
    if (isSuccess && result && resultRegionRef.current) {
  ...
  if (isSuccess && result) {
    ...
    return (
      <div className="container max-w-2xl mx-auto px-4 py-12 sm:py-20" id="sit-result">
        <motion.div
          ref={resultRegionRef}
          role="region"
          aria-live="polite"
          aria-label="SIT 테스트 결과"
          tabIndex={-1}
        ...
        >
          ...
          <Card className="p-8 bg-card shadow-xl border-t-4 border-t-primary mt-8">
            <h3 className="text-2xl font-bold mb-4 font-display">{typeInfo?.nickname}</h3>
            ...
            <div className="mt-6 pt-6 border-t">
              ...
              <div className="max-w-xs mx-auto touch-manipulation">
                <ShareCard
                  variant="SIT"
                  sitType={result.sitType}
                  keywords={typeInfo?.keywords ? [...typeInfo.keywords] : []}
  ...
  return (
    <div className="container max-w-xl mx-auto px-4 py-12 sm:py-20 md:py-32">
      <div className="space-y-8" role="main" aria-label="스타일 정체성 테스트">
        <div className="space-y-2" aria-live="polite" aria-atomic="true">
          <div className="flex justify-between text-sm text-muted-foreground font-medium">
            <span>질문 {currentStep + 1} / {questions.length}</span>
            <span aria-hidden="true">{Math.round(progress)}%</span>
          </div>
          <Progress value={progress} className="h-2" aria-valuenow={Math.round(progress)} aria-valuemin={0} aria-valuemax={100} aria-label={`진행률 ${Math.round(progress)}%`} />
          <span className="sr-only">질문 {currentStep + 1}번 of {questions.length}, 진행률 {Math.round(progress)}%</span>
        </div>
              ...
              <div className="grid gap-3 sm:gap-4">
                {currentQ.options.map((option) => (
                  <button
                    ...
                    className="w-full min-h-[48px] py-4 px-6 text-left rounded-xl border bg-card hover:border-primary hover:shadow-md transition-all duration-200 group touch-manipulation"
